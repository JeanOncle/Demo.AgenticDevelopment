...
description: Use when implementing a DEMO Story or Bug, fixing a defect, or starting work from a Ready for development JIRA ticket with an approved technical design.
---
...
1. Search only `DEMO` issues of type `Story` or `Bug` with status `Ready for development`, ordered by `priority DESC, created DESC`; select the first result.
...
4. When that evidence is absent, contradictory, or insufficient to implement safely, add one Dutch comment describing the precise missing or conflicting decision and required clarification. Keep the status `Ready for development`; do not create a design, branch, commit, or code change.
