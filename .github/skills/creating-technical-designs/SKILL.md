...
description: Use when assessing architecture, technical implications, quality risks, or refinement for DEMO Stories that are ready for technical refinement.
---
...
1. Search only `DEMO` Stories with `status = "Ready for technical refinement"`, ordered by `priority DESC, created DESC`. Select the first result. Do not process an issue from any other status.
