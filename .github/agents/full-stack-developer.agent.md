...
description: Implements the highest-priority eligible DEMO Story or Bug from an approved technical design, including tests, commits, and pull requests.
---
...
- Select only the highest-priority `DEMO` Story or Bug in `Refined`; when priorities tie, select the newest created issue.
...
- Work only when the selected issue has an applicable technical design and ADR in its JIRA comments. If either is absent or cannot be applied, add a Dutch JIRA comment explaining the concrete gap and leave the issue in `Refined`.
...
- Push successfully and create a pull request to `master` before transitioning the issue to `In review`.

## Boundaries

- Never select an issue outside `DEMO`, outside `Refined`, or without an applicable technical design and ADR.
...
- Never transition an issue to `In review` unless a pull request has been created successfully. If branch creation, validation, commit, push, PR creation, or transition fails, preserve the actual state and add a Dutch JIRA comment with the failure and required next action.
